...
- One endpoint (`http://127.0.0.1:20128/v1`), one credential pool. Chef's
  provider keys point at OmniRoute instead of vendor endpoints, so provider
  accounts rotate in the gateway dashboard — never in Atlas code.
